...
export default function KpiCard({ label, dotColor, value, delta, deltaType, footer, highlighted }: KpiCardProps) {
  return (
    <div className="card kpi-card" style={{
      ...
      <div className="mono" style={{ fontSize: "var(--text-xl)", fontWeight: 600, color: "var(--t1)", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis" }}>{value}</div>
      {delta && (
      ...
      {footer && (
        <div style={{ fontSize: "var(--text-xs)", color: "var(--t3)", marginTop: 4, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis" }}>{footer}</div>
      )}
    </div>
  );
}
